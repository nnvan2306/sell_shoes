...
const Cart = forwardRef((props, _ref) => {
    ...
    function calculateTotal() {
        if (currentUser) {
            preTotal = cartProducts?.reduce(
                (total, item) => total + item.productPrice * item.quantity,
                0
            );
            setPreTotal(preTotal);
        } else {
            preTotal = cartProductsNonUser?.reduce(
                (total, item) => total + item.productPrice * item.quantity,
                0
            );
            setPreTotal(preTotal);
    ...
    async function handleCheckVoucher() {
        const voucher = vouchers.find(
            (item) => item.voucherCode === discountCode
        );
                    ...
                    notify(
                        ...
                        "Hãy chi thêm " +
                            new Intl.NumberFormat("vi-VN", {
                                style: "currency",
                                currency: "VND",
                            }).format(voucher.minPrice - preTotal) +
    ...
    return (
        <>
                ...
                {vouchers?.map((item, index) => {
                    return (
                        <div key={index} style={{ margin: "0px 10px" }}>
                            <VoucherItem
                                active={
                                    item.voucherCode === voucherAL?.voucherCode
                                }
                                props={item}
                                onClickVoucher={() =>
                                    setDiscountCode(item.voucherCode)
                                }
                            />
                        </div>
                    );
            ...
            {currentUser && (
                <div
                    style={{ display: "flex", margin: "14px 0" }}
                ...
                >
                    ...
                    >
                        ...
                        <span style={{ color: "#2f5acf", fontSize: "14px" }}>
                            Ví voucher
                        </span>
                    </div>
                </div>
            ...
            <div className={cx("outerMoney")}>
                ...
                <span className={cx("money")}>
                    ...
                    {new Intl.NumberFormat("vi-VN", {
                        ...
                        isPercent ? (preTotal * discount) / 100 : discount
                    )}
                </span>
            </div>
            <div className={cx("outerMoney")}>
            ...
            <div className={cx("outerMoney")}>
                ...
                <span className={cx("money", "totalMoney")}>
                    {new Intl.NumberFormat("vi-VN", {
                        style: "currency",
                        currency: "VND",
                    }).format(
                        isPercent
                            ? preTotal - (preTotal * discount) / 100 + delivery
                            : preTotal - discount + delivery
                    )}
                </span>
            </div>
            {currentUser && (
