...
function Collection() {
    const listPrices = [
        { id: 0, name: "Tất cả giá", min: null, max: null },
    ...
    useEffect(() => {
        if (id)
            if (!id.includes("type")) {
                const listFilter = listProducts.filter(
                    (item) => item.productCategory === id
                );
                ...
                const colors = listFilter
                    ...
                    .reduce((acc2, e2) => {
                        if (
                            acc2.find(
                                (item) => item.colorName === e2.colorName
                            ) === undefined
                        ) {
                            acc2.push(e2);
                        }
    ...
    useEffect(() => {
        if (!id.includes("type")) {
            if (
                conditions.size.length === 0 &&
                conditions.color === "" &&
                conditions.type.length === 0 &&
                conditions.price &&
                Object.keys(conditions.price).length === 0
            ) {
                ...
                if (conditions.type.length !== 0) {
                    listFilter = currentProducts.filter((item) =>
                        conditions.type?.includes(item.productType)
                    );
                }
                    ...
                    for (let i = 0; i < listFilter.length; i++) {
                        ...
                        for (let j = 0; j < colorsSizes.length; j++) {
                            ...
                            sizes = sizes.filter((size) =>
                                conditions.size.includes(size.sizeName)
                            ); //filter size
                ...
                if (
                    ...
                    listFilter = listFilter.filter((item) => {
                        const mainPrice =
                            (item?.exportPrice * (100 - item?.discountPerc)) /
                            100;
                        if (conditions.price.min) {
                            if (conditions.price.max) {
                                if (
                                    mainPrice > conditions.price.min &&
                                    mainPrice < conditions.price.max
                                )
                                    return true;
                                return false;
                            } else {
                ...
                if (
                    ...
                    listFilter = listFilter.filter((item) => {
                        ...
                        if (conditions.price.min) {
                            ...
                            if (mainPrice < conditions.price.max) return true;
                            return false;
                        }
                    });
    ...
    const handleClickType = (type, index) => {
        ...
        setCategory((prev) => {
            state = {
                ...prev,
                type: prev.type.map((item, index2) => {
                    if (index2 === index)
    ...
    const handleClickSize = (size, index) => {
        ...
        setCategory((prev) => {
            state = {
                ...prev,
                size: prev.size.map((item, index2) => {
                    if (index2 === index)
                        return { ...item, checked: !item.checked };
                    else return item;
                }),
        ...
        <div className={cx("wrapper")}>
                                ...
                                {listPrices.map((item, index) => {
                                    return (
                                        <div
                                            ...
                                            className={cx(
                                                "outer-filter-select-price__item"
                                            )}
                                        >
                                            <div
                                                className={cx(
                                                    "filter-select-price__check"
                                                )}
                                            >
                                                <div
                                                    className={cx(
                                                        "filter-select-price__dot"
                                                    )}
                                                ></div>
                                                {conditions?.price?.id ===
                                                    item.id && (
            ...
            <div className={cx("right-side")}>
                <h1 className={cx("heading")}>
                    {id.includes("type") ? id.replace("type=", "") : id}
                </h1>
                        ...
                        {condititonsSelected.map((item, index) => {
                            return (
                                ...
                                >
                                    ...
                                    <span
                                        onClick={() =>
                                            handleClickRemoveSelected(item)
                                        }
                                        style={{
                                            fontSize: "12px",
                                            marginLeft: "5px",
